...
for (i in 1:length(statevec)) {
  ...
  if (nrow(df2) > 0) {
    ...
    df$alzheimers.roll <- round(na.approx(df$alzheimers.roll, na.rm=F))
    ...
    gfit <- try(glm.nb(alzheimers.roll ~ time + time_sq + cos1 + sin1 + season1 + season2 + season3 + season4 + season5 + season6,
    ...
    if (!inherits(gfit, "try-error")) {
      ...
      rr <- try(negpos(fitdata$alzheimers.roll, pred, upr, lwr, fitdata$date, sname))
    ...
    fitlist[[i]] <- data.frame(
      date=df$date,
      alzheimers.roll=fitdata$alzheimers.roll,
...
fitdata <- fitlist %>%
  bind_rows %>% 
  mutate(alzheimers.roll=alzheimers.roll/population *100000,
         pred=pred/population *100000,
         baseline=baseline/population *100000,
         lwr=lwr/population *100000,
         upr=upr/population *100000)

alzheimers_baseline_nb_naapprox <- fitdata %>%
  filter(date >= '2020-03-01') %>% 
  dplyr::select(region, pred, lwr, upr) %>% 
  group_by(region) %>% 
  summarize(baseline_alzheimers = sum(pred, na.rm = TRUE),
            baseline_alzheimers.lwr = sum(lwr, na.rm = TRUE),
            baseline_alzheimers.upr = sum(upr, na.rm = TRUE)
  )

save('alzheimers_baseline_nb_naapprox', file="../data/baselines/alzheimers_baseline_nb_naapprox2022.rda")

write.table(resdata, "../data/excess/excess alzheimers nb_naapprox2022.csv", sep=",", row.names =F)

alzheimers.roll <- fitdata %>%
  mutate(region=factor(region, levels=c("United States", state.name)))

g1 <- ggplot(subset(alzheimers.roll,date>"2018-08-01")) +
  geom_vline(xintercept=as.Date("2020-03-01"), lty=3, col="red") +
  geom_line(aes(date, alzheimers.roll), lwd=0.8) +
  ...
  #scale_y_log10("Alzheimers mortality per 100,000", limits = c(0.1, 2), expand=c(0, 0),
  #              breaks=c(0.1, 0.5, 1, 1.5)) +
...
ggsave("../figures/S2. alzheimers_ggplot (NB)_naapprox2022.png", g1, width=13, height=9)
save('alzheimers.roll', file="../data/excess/national weekly alzheimers_excess_naapprox2022.rda")
