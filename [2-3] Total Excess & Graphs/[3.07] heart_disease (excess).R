...
for (i in 1:length(statevec)) {
  ...
  if (nrow(df2) > 0) {
    ...
    df$heart_disease.roll <- round(na.approx(df$heart_disease.roll, na.rm=F))
    ...
    gfit <- try(glm.nb(heart_disease.roll ~ time + time_sq + cos1 + sin1 + season1 + season2 + season3 + season4 + season5 + season6,
    ...
    if (!inherits(gfit, "try-error")) {
      ...
      rr <- try(negpos(fitdata$heart_disease.roll, pred, upr, lwr, fitdata$date, sname))
    ...
    fitlist[[i]] <- data.frame(
      date=df$date,
      heart_disease.roll=fitdata$heart_disease.roll,
...
fitdata <- fitlist %>%
  bind_rows %>% 
  mutate(heart_disease.roll=heart_disease.roll/population *100000,
         pred=pred/population *100000,
         baseline=baseline/population *100000,
         lwr=lwr/population *100000,
         upr=upr/population *100000)

heart_disease_baseline_nb_naapprox <- fitdata %>%
  filter(date >= '2020-03-01') %>% 
  dplyr::select(region, pred, lwr, upr) %>% 
  group_by(region) %>% 
  summarize(baseline_heart_disease = sum(pred, na.rm = TRUE),
            baseline_heart_disease.lwr = sum(lwr, na.rm = TRUE),
            baseline_heart_disease.upr = sum(upr, na.rm = TRUE)
  )

save('heart_disease_baseline_nb_naapprox', file="../data/baselines/heart_disease_baseline_nb_naapprox2022.rda")

write.table(resdata, "../data/excess/excess heart_disease nb_naapprox2022.csv", sep=",", row.names =F)

heart_disease.roll <- fitdata %>%
  mutate(region=factor(region, levels=c("United States", state.name)))



g1 <- ggplot(subset(heart_disease.roll,date>"2018-08-01")) +
  geom_vline(xintercept=as.Date("2020-03-01"), lty=3, col="red") +
  geom_line(aes(date, heart_disease.roll), lwd=0.8) +
...
ggsave("../figures/S6. heart_disease_ggplot (NB)_naapprox2022.png", g1, width=13, height=9)
save('heart_disease.roll', file="../data/excess/national weekly heart_disease_excess_naapprox2022.rda")
