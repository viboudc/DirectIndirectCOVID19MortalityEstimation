...
### This is the excess mortality burden estimation step, for the resp+COVID time series
### We can only do this for 17 states with sufficient weekly counts
...
for (i in 1:length(statevec)) {
  ...
  if (nrow(df2) > 0) {
    ...
    df$resp.covid.roll <- round(na.approx(df$resp.covid.roll, na.rm=F))
    ...
    gfit <- try(glm.nb(resp.covid.roll ~ time + time_sq + cos1 + sin1 + season1 + season2 + season3 + season4 + season5 + season6,
    ...
    if (!inherits(gfit, "try-error")) {
      ...
      rr <- try(negpos(fitdata$resp.covid.roll, pred, upr, lwr, fitdata$date, sname))
    ...
    fitlist[[i]] <- data.frame(
      date=df$date,
      resp.covid.roll=fitdata$resp.covid.roll,
...
# for Table 1
resp.covid_t1 <- reslist %>%
  bind_rows %>% 
  mutate(excess = signif(excess, 3),
         excess.lwr = signif(excess.lwr,3),
         excess.upr = signif(excess.upr,3),
         resp.data_CI = paste("(", paste(excess.lwr, excess.upr, sep = ", ", collapse = NULL), ")", sep = "", collapse = NULL),
         resp.data = paste(excess, resp.data_CI, sep = " ", collapse = NULL)) %>% 
  dplyr::select(region, resp.data)

save('resp.covid_t1', file="resp.covid_t1.rda")

### Saves model baseline and mortality observations for time series graphs

fitdata <- fitlist %>%
  bind_rows %>% 
  mutate(resp.covid.roll=resp.covid.roll/population *100000,
         pred=pred/population *100000,
         baseline=baseline/population *100000,
         lwr=lwr/population *100000,
         upr=upr/population *100000)

resp.covid_baseline_nb_naapprox <- fitdata %>%
  filter(date >= '2020-03-01') %>% 
  dplyr::select(region, pred, lwr, upr) %>% 
  group_by(region) %>% 
  summarize(baseline_resp.covid = sum(pred, na.rm = TRUE),
            baseline_resp.covid.lwr = sum(lwr, na.rm = TRUE),
            baseline_resp.covid.upr = sum(upr, na.rm = TRUE)
            )

save('resp.covid_baseline_nb_naapprox', file="../data/baselines/resp.covid19_baseline_nb_naapprox2022.rda")

write.table(resdata, "../data/excess/excess resp.covid19 nb_naapprox2022.csv", sep=",", row.names =F)

resp.covid.roll <- fitdata %>%
  mutate(region=factor(region, levels=c("United States", state.name)))

g1 <- ggplot(subset(resp.covid.roll,date>"2018-08-01")) +
  geom_vline(xintercept=as.Date("2020-03-01"), lty=3, col="red") +
  geom_vline(xintercept=c(as.Date("2019-01-01"),as.Date("2020-01-01"),as.Date("2021-01-01"),as.Date("2022-01-01")), lty=3) +
    geom_line(aes(date, resp.covid.roll), lwd=0.8) +
...
ggsave("../figures/S8. resp.covid19_ggplot (NB)_naapprox2022.png", g1, width=9, height=7)
save('resp.covid.roll', file="../data/excess/national weekly resp.covid19_excess_naapprox2022.rda")
