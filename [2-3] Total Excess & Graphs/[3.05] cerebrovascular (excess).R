...
for (i in 1:length(statevec)) {
  ...
  if (nrow(df2) > 0) {
    ...
    df$cerebrovascular.roll <- round(na.approx(df$cerebrovascular.roll, na.rm=F))
    ...
    gfit <- try(glm.nb(cerebrovascular.roll ~ time + time_sq + cos1 + sin1 + season1 + season2 + season3 + season4 + season5 + season6,
    ...
    if (!inherits(gfit, "try-error")) {
      ...
      rr <- try(negpos(fitdata$cerebrovascular.roll, pred, upr, lwr, fitdata$date, sname))
    ...
    fitlist[[i]] <- data.frame(
      date=df$date,
      cerebrovascular.roll=fitdata$cerebrovascular.roll,
...
fitdata <- fitlist %>%
  bind_rows %>% 
  mutate(cerebrovascular.roll=cerebrovascular.roll/population *100000,
         pred=pred/population *100000,
         baseline=baseline/population *100000,
         lwr=lwr/population *100000,
         upr=upr/population *100000)

cerebrovascular_baseline_nb_naapprox <- fitdata %>%
  filter(date >= '2020-03-01') %>% 
  dplyr::select(region, pred, lwr, upr) %>% 
  group_by(region) %>% 
  summarize(baseline_cerebrovascular = sum(pred, na.rm = TRUE),
            baseline_cerebrovascular.lwr = sum(lwr, na.rm = TRUE),
            baseline_cerebrovascular.upr = sum(upr, na.rm = TRUE)
  )

save('cerebrovascular_baseline_nb_naapprox', file="../data/baselines/cerebrovascular_baseline_nb_naapprox2022.rda")

write.table(resdata, "../data/excess/excess cerebrovascular nb_naapprox2022.csv", sep=",", row.names =F)

cerebrovascular.roll <- fitdata %>%
  mutate(region=factor(region, levels=c("United States", state.name)))

g1 <- ggplot(subset(cerebrovascular.roll,date>"2018-08-01")) +
  geom_vline(xintercept=as.Date("2020-03-01"), lty=3, col="red") +
  geom_line(aes(date, cerebrovascular.roll), lwd=0.8) +
...
ggsave("../figures/S4. cerebrovascular_ggplot (NB)_naapprox2022.png", g1, width=13, height=9)
save('cerebrovascular.roll', file="../data/excess/national weekly cerebrovascular_excess_naapprox2022.rda")
