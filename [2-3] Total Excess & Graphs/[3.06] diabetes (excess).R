...
statevec=statevec[-which(statevec=="Nevada")]
...
for (i in 1:length(statevec)) {
  ...
  if (nrow(df2) > 0) {
    ...
    df$diabetes.roll <- round(na.approx(df$diabetes.roll, na.rm=F))
    ...
    gfit <- try(glm.nb(diabetes.roll ~ time + time_sq + cos1 + sin1 + season1 + season2 + season3 + season4 + season5 + season6,
    ...
    if (!inherits(gfit, "try-error")) {
      ...
      rr <- try(negpos(fitdata$diabetes.roll, pred, upr, lwr, fitdata$date, sname))
    ...
    fitlist[[i]] <- data.frame(
      date=df$date,
      diabetes.roll=fitdata$diabetes.roll,
...
fitdata <- fitlist %>%
  bind_rows %>% 
  mutate(diabetes.roll=diabetes.roll/population *100000,
         pred=pred/population *100000,
         baseline=baseline/population *100000,
         lwr=lwr/population *100000,
         upr=upr/population *100000)

diabetes_baseline_nb_naapprox <- fitdata %>%
  filter(date >= '2020-03-01') %>% 
  dplyr::select(region, pred, lwr, upr) %>% 
  group_by(region) %>% 
  summarize(baseline_diabetes = sum(pred, na.rm = TRUE),
            baseline_diabetes.lwr = sum(lwr, na.rm = TRUE),
            baseline_diabetes.upr = sum(upr, na.rm = TRUE)
  )

save('diabetes_baseline_nb_naapprox', file="../data/baselines/diabetes_baseline_nb_naapprox2022.rda")

write.table(resdata, "../data/excess/excess diabetes nb_naapprox2022.csv", sep=",", row.names =F)

diabetes.roll <- fitdata %>%
  mutate(region=factor(region, levels=c("United States", state.name)))

g1 <- ggplot(subset(diabetes.roll,date>"2014-08-01")) +
  geom_vline(xintercept=as.Date("2020-03-01"), lty=3, col="red") +
  geom_line(aes(date, diabetes.roll), lwd=0.8) +
...
ggsave("../figures/S5. diabetes_ggplot (NB)_naapprox2022.png", g1, width=13, height=9)
save('diabetes.roll', file="../data/excess/national weekly diabetes_excess_naapprox2022.rda")
